...
### Agent self-install entrypoint

This archive can install itself for Codex and Claude Code. When the user asks to install, enable, update, verify, or remove it, read `SELF_INSTALL.md` and use `scripts/agent_install.py`. Default an unspecified request to project scope, run `--dry-run` first, never use `--force` without explicit permission, and run `verify` afterward. The installer is local-only and must not be replaced with an improvised network or package-manager workflow.
